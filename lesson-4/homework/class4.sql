...
Select EmployeeID, Age, ISNULL(Email,'noemail@example.com' ) as Email from Employees;
...
Select EmployeeID, coalesce(FirstName , Lastname, 'Unknown') as Name from Employees;
--Write a query that selects distinct Category and Price from the Products table
Select distinct Category from Products_Discounted
Select distinct Price from Products_Discounted
...
--select WHERE clause to filter for employees whose Age is less than 30 and who work in either the 'HR' or 'IT' department.
Select EmployeeID, FirstName, LastName, Age, DepartmentName from Employees 
...
--(Search how to get the current date and the difference betweeen two dates)
