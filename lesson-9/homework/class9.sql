...
SELECT e.name, e.salary, m.name AS managername, m.salary AS manager_salary
FROM Employees e
...
SELECT p.paymentid, p.amount, o.orderid, o.quantity, pr.price, 
       (o.quantity * pr.price) AS calculated_amount
FROM Payments p
...
SELECT m.name AS manager_name, m.salary AS manager_salary, e.name 
AS employee_name, e.salary AS employee_salary
FROM Employees m
