...
Select *,
concat(employee_id, '-', FIRST_NAME, ' ', LAST_NAME) as Full_name
from Employees
...
Select *,
replace(phone_number, 124,999) as newphone
from Employees 
...
INSERT INTO TestMax VALUES (2001,10,101,87) ,(2002,103,19,88) ,(2003,21,23,89),(2004,27,28,91)
Select * from TestMax
Select year1, max(max_value) from
(
Select year1,(max1) as max_value from TestMax
union all
...
on e1.DEPARTMENT_ID=dept_avrg.DEPARTMENT_ID
where e1.SALARY>dept_avrg.Avgsalary
...
with cte1 as (
Select DEPARTMENT_ID, count(Employee_id) as Longtermcount
from Employees 
Where HIRE_DATE<DATEADD(year,-3,Getdate())
group by DEPARTMENT_ID
),
cte2 as (
Select department_id, count(Employee_id) Countofemp
from Employees
group by DEPARTMENT_ID
)
Select cte1.DEPARTMENT_ID, 
(Longtermcount/Countofemp)*100 as Percentge from cte1
join cte2
...
INSERT INTO Personal (SpacemanID, JobDescription, MissionCount) VALUES(1001,'Astrogator',6),(2002,'Astrogator',12),(3003,'Astrogator',17), 
(4004,'Geologist',21),(5005,'Geologist',9),(6006,'Geologist',8), (7007,'Technician',13),(8008,'Technician',2),(9009,'Technician',7);
Select * from Personal
Select Jobdescription, Max(missioncount)Maxcount, Min(missioncount)Mincount
from Personal
...
INSERT INTO cinema (id, movie, description, rating) VALUES (1, 'War', 'great 3D', 8.9),(2, 'Science', 'fiction', 8.5), 
...
INSERT INTO SingleOrder VALUES(0,'All'), (1,'Pawan'), (2,'Avtaar'), (3,'Kishan'), (4,'Vaibhav'), (5,'Ashutosh')
...
INSERT INTO person VALUES (1, '111-11-1111', 'aa155', '123itin'),(2, NULL, 'aa156', '124itin'),(3, NULL, NULL, '125itin'),(4, NULL, NULL, NULL),
(5, '222-22-2222', NULL, '126itin'), (6, '333-33-3333', 'aa157', NULL)
Select * from person
...
Round(Datediff(year,hire_date,getdate()),2) AS Yearsofservice
from Employees
...
Select e1.department_id, e1.salary,Avgsalary
from Employees e1
join  
(
Select DEPARTMENT_ID, Round(Avg(salary),0) as Avgsalary 
from Employees
group by DEPARTMENT_ID
) as dept_avrg
on e1.DEPARTMENT_ID=dept_avrg.DEPARTMENT_ID
...
CREATE TABLE Student (StudentName VARCHAR(50) PRIMARY KEY,Birthday DATE); --GO
INSERT INTO Student (StudentName, Birthday) VALUES('Susan', '2015-04-15'),
('Tim', '2015-04-15'),('Jacob', '2015-04-15'),('Earl', '2015-02-05'),
('Mike', '2015-05-23'),('Angie', '2015-05-23'),('Jenny', '2015-11-19'),
('Michelle', '2015-12-12'),('Aaron', '2015-12-18');
Select * from Student
create table Students( StudentID int, FullName varchar(100),Grade decimal(5,2))
insert into Students values (1,'James Alexander Hamilton',98.5),(2,'Arthur George Mason',78.2),
(3,'Lily B Peters',81),(4,'Kayne Thomas Derek',87.3),(5,'April Dukane Paris',79.1),(6,'Payton Carl Johnson',97.1)
...
INSERT INTO Equations (Equation) VALUES('123'),('1+2+3'),('1+2-3'),('1+23'),('1-2+3'),('1-2-3'),('1-23'),('12+3'),('12-3');
...
INSERT INTO DMLTable (SequenceNumber, String) VALUES(1,'SELECT'),(2,'Product,'),(3,'UnitPrice,'),(4,'EffectiveDate'),(5,'FROM'), (6,'Products'),(7,'WHERE'),(8,'UnitPrice'),(9,'> 100')
Select * from DMLTable
Select 
string_agg(string ,' ')within group (order by sequencenumber) as reconquery
from DMLTable
...
SELECT
    ...
    FullName,
    Grade,
    LAG(Grade, 1, 0) OVER (ORDER BY StudentID) AS PreviousGrade,
    Grade + LAG(Grade, 1, 0) OVER (ORDER BY StudentID) AS SumWithPrevious
FROM Students
ORDER BY StudentID;
...
JOIN (
    ...
    HAVING COUNT(*) > 1
) AS SameBirthdays ON CAST(s1.Birthday AS DATE) = SameBirthdays.BirthDate
ORDER BY CAST(s1.Birthday AS DATE);
