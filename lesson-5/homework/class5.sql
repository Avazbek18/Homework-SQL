...
Select ProductName as Name, ProductID, Price, Category,StockQuantity from Products;
...
--3.Use UNION to combine results from 2 queries that select ProductName from Products and ProductName from Products_Discounted.
...
--9. Use UNION to combine results from two queries that select ProductName from Products and ProductName from 
--OutOfStock tables.
...
Except
Select * from Products_Discounted
--11.Create a conditional column using IIF that shows 'Expensive' if the Price is greater than 1000, 
--and 'Affordable' if less, from Products table.
Select *,  IIF( Price > 1000, 'Expensive' , 'Affordabe')
From Products
...
INTERSECT
SELECT ProductID, ProductName, Price FROM Products_Discounted;
--15.Write a query that uses CASE to assign 'Top Tier' if SaleAmount > 500, 'Mid Tier' if SaleAmount BETWEEN 200 AND 
--500, and 'Low Tier' otherwise. (From Sales table)
CREATE TABLE Sales (
...
--17. Write a query that uses a CASE statement to determine the discount percentage based on the quantity purchased. 
--Use orders table. Result set should show customerid, quantity and discount percentage. 
--The discount should be applied as follows: More than 1 item: 3% Between 1 and 3 items : 5% Otherwise: 7%
